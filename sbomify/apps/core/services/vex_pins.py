...
def ensure_latest_vex_pinned(release: Any, component: Any) -> bool:
    ...
    or when the newest VEX is already pinned. Returns True when the pin
    changed.
    """
    ...
    vex_pins.filter(auto_pinned=True).delete()
...
def refresh_vex_pins_for_component(component_id: str) -> int:
    ...
    for release in releases:
        if ensure_latest_vex_pinned(release, component):
            changed += 1
    return changed
